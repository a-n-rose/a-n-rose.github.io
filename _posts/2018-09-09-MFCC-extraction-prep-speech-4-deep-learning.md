...
Regarding how the MFCC data was processed by each of the networks, for the ANN, each sample of the MFCC data was processed, but not in succession. The 2 million rows were separated into trainging and testing data: 1,600,000 in training, 400,000 in testing. The language types were equally represented. The training data were applied to the neural network at random. For testing model performance, test MFCC data were used. The neural network had to classify (via sigmoid function) whether the 40 MFCCs belonged to that of an English speaker or German speaker, and eventually also Russian speaker. To test on new data, the classifer had to classify each 40 MFCC sample of the new signal as either German or English; how the ANN labeled most of the MFCC samples determined the label the ANN classified the speech as. 
