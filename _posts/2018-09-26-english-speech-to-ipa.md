...
Even though I only used 5 minutes of speech data, that worked out to be 26,280 samples of MFCC data for the condition without overlap and 76,640 samples of MFCC data for the condition with overlap. Keep in mind, because I used 40 MFCCs, each sample had 40 features. I could not feed the data all at once to the LSTM model and instead developed a generator function that fed the network 20 sequences of MFCC data at a time, which corresponded to the MFCC samples with 3 IPA labels (see Table 3). To see the code relevant for the building of the LSTM, please refer to the script <a href="https://github.com/a-n-rose/language-classifier/blob/master/english_speech_to_ipa/batch2model.py">'batch2model.py'</a>, which imports the 'sql_data.py' and 'batch_prep.py' scripts mentioned earlier.
